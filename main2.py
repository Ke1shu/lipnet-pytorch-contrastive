...
def test(model, net, lossfunc, file_list=None):

    # file_list を渡さない場合は val_list を使う（train_list を誤って参照していたため修正）
...
def split_file_list(file_list_path, val_ratio=0.8, seed=0, out_prefix=None):
    """txt のファイルリストを val/test に分割して、分割後の txt パスを返す。

    - file_list_path: 例) data/unseen_val.txt
    - val_ratio: val に割り当てる割合（残りが test）
    - seed: シャッフルの乱数シード（再現性用）
    - out_prefix: 出力ファイルの prefix（None の場合は元ファイル名から生成）
    """
    # 読み込み（空行は除外）
    ...
    if len(items) < 2:
        raise ValueError(f"file_list too small to split: {file_list_path} (n={len(items)})")

    rng = random.Random(seed)
    rng.shuffle(items)

    n_val = int(round(len(items) * val_ratio))
    n_val = max(1, min(len(items) - 1, n_val))  # 1 <= n_val <= n-1

    val_items = items[:n_val]
    test_items = items[n_val:]

    base_dir = os.path.dirname(file_list_path)
    base_name = os.path.splitext(os.path.basename(file_list_path))[0]
    if out_prefix is None:
        out_prefix = os.path.join(base_dir, base_name + f"_split_{int(val_ratio*100)}_{seed}")

    val_path = out_prefix + "_val.txt"
    test_path = out_prefix + "_test.txt"
...
def combinationLoss(model, net):
    ...
    for epoch in range(opt.max_epoch):
        ...
        for i_iter, input in enumerate(loader):
            ...
            vids = input.get('vids')
            vid1, vid2 = vids[0].cuda(), vids[2].cuda() #0 and 90
            txt = input.get('txt').cuda()
            tg = textgrid2dic(input.get('phonem'))
            ...
            ctc_loss = ctc_crit(
                y2.transpose(0, 1).log_softmax(-1),  # y2 90を使用
                txt,
            ...
            #ログ
            if tot_iter % opt.test_step == 0:
                val_loss, val_wer, val_cer = test(model, net, "combi", file_list=val_list_path)
                test_loss, test_wer, test_cer = test(model, net, "combi", file_list=test_list_path)
